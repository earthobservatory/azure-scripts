...
2. Edit either the `dev_var_values.tfvars` or the `prod_var_values.tfvars` files for development or production respectively to suit your configuration.

3. Run `terraform init` in order for Terraform to download the appropriate tools for working with Azure.

4. Run `terraform apply -var-file=[dev_var_values.tfvars/prod_var_values.tfvars]`, verify that the parameters are correct and type `yes` to apply the changes.
